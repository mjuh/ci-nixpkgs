...
let
  ...
  testDiffPy = phpVersion: writeScript "test-php-diff.py" ''
    ...
    # Original JSON
    with open('${./. + "/${phpVersion}.json"}') as json_file:
    ...
    with open('/tmp/xchg/coverage-data/deepdiff.html', 'w') as outfile:
      outfile.write(DeepDiff(data1, data2, ignore_order=True).to_json())
 '';

  testDiffPyWithExcludes = phpVersion: writeScript "test-php-diff-with-excludes.py" ''
    #!/${python-with-my-packages}/bin/python
    ...
    # Generate diff result
    with open('/tmp/xchg/coverage-data/deepdiff-with-excludes.html', 'w') as outfile:
...
import maketest ({ pkgs, lib, ... }: {
  ...
  testScript = [
    ''
          ...
          $docker->succeed("${testDiffPy phpVersion}");
          $docker->succeed("${testDiffPyWithExcludes phpVersion}");
