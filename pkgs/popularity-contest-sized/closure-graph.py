...
def reorder_by_min_size(paths, size_map, min_size):
    t1, t2 = tee(paths)
    pred = lambda x: size_map[x] >= min_size
    return list(chain(filter(pred, t1), filterfalse(pred, t2)))
...
def main():
    ...
    # ]
    graph = data[key]
    nar_sizes = {each['path']: each['narSize'] for each in graph}
